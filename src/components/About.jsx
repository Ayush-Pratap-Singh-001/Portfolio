    ...
    <section id="about" style={{ padding: '100px 1.5rem' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
                ...
                <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 800, marginBottom: '1rem', color: 'var(--text-primary)' }}>About Me</h2>
            ...
            >
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        >
                            <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--accent-green)', display: 'inline-block', flexShrink: 0 }} />
                            Open to work
                        </motion.div>
                    </div>
                </div>
                ...
                <div>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', lineHeight: 1.9, marginBottom: '1.25rem' }}>
                        Hey! I'm <span style={{ color: 'var(--accent-blue)', fontWeight: 600 }}>Ayush</span>, a software engineering student at{' '}
                        <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>Lovely Professional University</span>, passionate about building backend systems that are fast, reliable and maintainable.
                    </p>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', lineHeight: 1.9, marginBottom: '1.75rem' }}>
                        I thrive at the intersection of <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>system design and clean code</span> — whether that's designing efficient database schemas, architecting RESTful APIs with Spring Boot, or experimenting with cloud infrastructure on AWS.
                    </p>
                    {/* Quick fact chips */}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem', marginBottom: '4rem' }}>
                {highlights.map(({ icon: Icon, title, desc }, i) => (
                    ...
                    >
                        ...
                        <div>
                            <h4 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.35rem', color: 'var(--text-primary)' }}>{title}</h4>
                            <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: 1.7 }}>{desc}</p>
                        </div>
                    </motion.div>
                ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem' }}>
                {Stats.map(({ value, label, color }, i) => (
                    <motion.div
                        ...
                        style={{ textAlign: 'center', padding: '1.5rem', background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 14, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}
                    >
                        <div style={{ fontSize: 'clamp(1.8rem, 3vw, 2.5rem)', fontWeight: 800, color, fontFamily: 'var(--font-heading)' }}>{value}</div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: 6 }}>{label}</div>
                    </motion.div>
                ))}
            </div>
        </div>
