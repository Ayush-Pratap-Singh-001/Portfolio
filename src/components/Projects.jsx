...
const PROJECTS = [
    {
        ...
        highlights: ['JWT-secured multi-role auth (Admin/Member)', 'Automated fine calculation engine', 'Full CRUD with Hibernate ORM & MySQL'],
        tech: ['Spring Boot', 'MySQL', 'JPA/Hibernate'],
        category: 'Backend',
        github: 'https://github.com/Ayush-Pratap-Singh-001/LibraryManagementSystem',
        live: '#',
        color: '#3b82f6',
    },
    {
        title: 'LeadGen ChatBot',
    ...
    {
        title: 'Memory Management Visualiser',
...
const ProjectModal = ({ project, onClose }) => (
    <motion.div
        ...
        exit={{ opacity: 0 }}
        onClick={onClose}
        style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', backdropFilter: 'blur(8px)' }}
    >
        <motion.div
            ...
            style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 20, maxWidth: 600, width: '100%', overflow: 'hidden', boxShadow: `0 0 60px ${project.color}25` }}
        >
            {/* Header */}
            <div style={{ padding: '1.5rem 1.5rem 1rem', borderBottom: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <button onClick={onClose} style={{ background: 'var(--surface-muted)', border: '1px solid var(--card-border)', color: 'var(--text-secondary)', borderRadius: 8, padding: '0.4rem', cursor: 'pointer', display: 'flex' }}>
                    <X size={18} />
                </button>
            </div>
            {/* Body */}
            <div style={{ padding: '1.5rem' }}>
                <p style={{ color: 'var(--text-secondary)', lineHeight: 1.8, marginBottom: '1.25rem' }}>{project.longDesc}</p>
                ...
                <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem' }}>
                    {project.highlights.map(h => (
                        <li key={h} style={{ display: 'flex', gap: 8, color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1.5rem' }}>
                    {project.tech.map(t => (
                        <span key={t} style={{ fontSize: '0.7rem', fontFamily: 'var(--font-mono)', background: 'var(--surface-muted)', border: '1px solid var(--card-border)', color: 'var(--text-secondary)', padding: '4px 10px', borderRadius: 6 }}>{t}</span>
                    ))}
                </div>
...
const ProjectCard = ({ project, onClick }) => (
    <motion.div
        ...
        whileHover={{ y: -6, boxShadow: `0 20px 50px ${project.color}20` }}
        onClick={onClick}
        style={{
    ...
    >
        ...
        {/* Thumbnail */}
        <div style={{ height: 140, background: `linear-gradient(135deg, ${project.color}15, ${project.color}05)`, display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid var(--card-border)' }}>
            <FolderCode size={48} style={{ color: `${project.color}60` }} />
        </div>

        <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: '0.75rem' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 700, lineHeight: 1.3, color: 'var(--text-primary)' }}>{project.title}</h3>
                <div style={{ display: 'flex', gap: 10 }} onClick={e => e.stopPropagation()}>
                    <a href={project.github} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)' }}><Github size={17} /></a>
                    <a href={project.live} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)' }}><ExternalLink size={17} /></a>
                </div>
            </div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', lineHeight: 1.7, marginBottom: '1rem', flex: 1 }}>{project.shortDesc}</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
                {project.tech.slice(0, 3).map(t => (
                    <span key={t} style={{ fontSize: '0.65rem', fontFamily: 'var(--font-mono)', background: `${project.color}18`, color: project.color, padding: '3px 8px', borderRadius: 5, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t}</span>
                ))}
                {project.tech.length > 3 && <span style={{ fontSize: '0.65rem', fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)', padding: '3px 8px' }}>+{project.tech.length - 3} more</span>}
            </div>
            <div style={{ marginTop: '1rem', fontSize: '0.72rem', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-end', gap: '1.5rem', marginBottom: '2.5rem' }}>
                    <div>
                        <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
                            style={{ color: 'var(--accent-blue)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.2em', display: 'block', marginBottom: '0.5rem' }}>
                            03. Portfolio
                        </motion.span>
                        <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 800, color: 'var(--text-primary)' }}>Featured Projects</h2>
                    </div>
                        ...
                        {FILTERS.map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                style={{
                                    ...
                                    background: filter === f ? 'var(--accent-blue)' : 'var(--card-bg)',
